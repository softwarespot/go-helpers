...
import (
	...
	"github.com/softwarespot/go-helpers/helpers"
...
func (s *Server) Handle(pattern string, handler Handler) {
	s.HandleFunc(pattern, func(w http.ResponseWriter, r *http.Request) error {
...
func (s *Server) ListenAndServe(ctx context.Context) error {
	...
	s.logger.Log("start graceful server shutdown", logging.LevelNotice,
		"address", s.server.Addr,
		"context-error", helpers.ErrorString(ctx.Err()),
	)
	err := s.server.Shutdown(ctx)
	s.logger.Log("stop graceful server shutdown", logging.LevelNotice,
		"address", s.server.Addr,
		"shutdown-error", helpers.ErrorString(err),
	)
	return err
}
