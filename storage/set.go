...
func NewSet[T comparable](s *Storage, name string) (*Set[T], error) {
	...
	err := execTransaction(s.db, func(tx *sql.Tx) error {
		...
		if _, err := tx.Exec(fmt.Sprintf(
			...
			return fmt.Errorf("storage.NewSet: create set key hash and expires at index: %w", err)
		}
		return nil
	})
	if err != nil {
...
func (s *Set[T]) add(funcName string, value T, expiration time.Duration) error {
	...
	if _, err := s.storage.db.Exec(query, hashedKey, encValue, getKeyExpiresAt(expiration), nowUnixMilli()); err != nil {
...
func execAddBatch(tx *sql.Tx, tableName, funcName string, placeholders []string, args []interface{}) error {
	query := fmt.Sprintf(
		`
            INSERT INTO %s (key_hash, value, expires_at, updated_at)
            VALUES %s
