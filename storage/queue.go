...
func NewQueue[T any](s *Storage, name string) (*Queue[T], error) {
	tableName := fmt.Sprintf("queue_%s", name)
	err := execTransaction(s.db, func(tx *sql.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("storage.NewQueue: create queue expires at index: %w", err)
		}
		return nil
	})
	if err != nil {
...
func (q *Queue[T]) enqueue(funcName string, value T, expiration time.Duration) error {
	...
	if _, err = q.storage.db.Exec(query, encValue, getKeyExpiresAtMilli(expiration), nowUnixMilli()); err != nil {
...
func (q *Queue[T]) Dequeue() (T, bool, error) {
	...
	err := execTransaction(q.storage.db, func(tx *sql.Tx) error {
		query := fmt.Sprintf(
			`
