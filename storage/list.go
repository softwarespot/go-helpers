...
func NewList[T any](s *Storage, name string) (*List[T], error) {
	tableName := fmt.Sprintf("list_%s", name)
	err := execTransaction(s.db, func(tx *sql.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("storage.NewList: create list expires at index: %w", err)
		}
		return nil
	})
	if err != nil {
...
func (l *List[T]) appendEx(funcName string, value T, expiration time.Duration) error {
	...
	if err := l.storage.db.QueryRow(query).Scan(&nextPos); err != nil {
...
func (l *List[T]) Get(position int) (T, bool, error) {
	...
	query := fmt.Sprintf(
		`
            SELECT value FROM %s
            WHERE position = ? AND (expires_at = 0 OR expires_at > ?)
        `,
		l.tableName,
	)
	var encValue []byte
...
func (l *List[T]) Delete(position int) error {
	...
	err = execTransaction(l.storage.db, func(tx *sql.Tx) error {
		tempTable := fmt.Sprintf("temp_%s_%d", l.tableName, time.Now().UnixNano())
		_, err := tx.Exec(fmt.Sprintf(
			`
		...
		_, err = tx.Exec(fmt.Sprintf(
			`
                ...
                FROM %s
            `,
			l.tableName, tempTable,
		))
		if err != nil {
			return fmt.Errorf("list.Remove: reindex values: %w", err)
		}

		_, err = tx.Exec(fmt.Sprintf(`DROP TABLE %s`, tempTable))
		if err != nil {
