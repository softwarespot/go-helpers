...
func NewPriorityQueue[T any](s *Storage, name string) (*PriorityQueue[T], error) {
	tableName := fmt.Sprintf("pqueue_%s", name)
	err := execTransaction(s.db, func(tx *sql.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("storage.NewPriorityQueue: create priority queue expires index: %w", err)
		}
		_, err = tx.Exec(fmt.Sprintf(
			`
				CREATE INDEX IF NOT EXISTS %s_priority_idx ON %s(priority)
			`,
			tableName,
			tableName,
		))
		if err != nil {
			return fmt.Errorf("storage.NewPriorityQueue: create priority queue priority index: %w", err)
		}
		return nil
	})
	if err != nil {
...
func (pq *PriorityQueue[T]) EnqueueEx(value T, priority int, expiration time.Duration) error {
	return pq.enqueueEx("EnqueueEx", value, priority, expiration)
}

func (pq *PriorityQueue[T]) enqueueEx(funcName string, value T, priority int, expiration time.Duration) error {
	...
	if _, err = pq.storage.db.Exec(
		...
		encValue,
		priority,
		getKeyExpiresAtMilli(expiration),
		nowUnixMilli(),
	); err != nil {
...
func (pq *PriorityQueue[T]) Dequeue() (T, bool, error) {
	...
	err := execTransaction(pq.storage.db, func(tx *sql.Tx) error {
		query := fmt.Sprintf(
			`
