...
func NewStack[T any](s *Storage, name string) (*Stack[T], error) {
	tableName := fmt.Sprintf("stack_%s", name)
	err := execTransaction(s.db, func(tx *sql.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("storage.NewStack: create stack expires at index: %w", err)
		}
		return nil
	})
	if err != nil {
...
func (s *Stack[T]) push(funcName string, value T, expiration time.Duration) error {
	...
	if _, err = s.storage.db.Exec(query, encValue, getKeyExpiresAtMilli(expiration), nowUnixMilli()); err != nil {
...
func (s *Stack[T]) Pop() (T, bool, error) {
	...
	err := execTransaction(s.storage.db, func(tx *sql.Tx) error {
		query := fmt.Sprintf(
			`
