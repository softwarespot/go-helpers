...
func NewMap[K comparable, V any](s *Storage, name string) (*Map[K, V], error) {
	...
	err := execTransaction(s.db, func(tx *sql.Tx) error {
		...
		if _, err := tx.Exec(fmt.Sprintf(
			`
				CREATE INDEX IF NOT EXISTS %s_hash_expires_idx ON %s(key_hash, expires_at)
			`,
			tableName,
			tableName,
		)); err != nil {
			return fmt.Errorf("storage.NewMap: create map key hash and expires at index: %w", err)
		}
		return nil
	})
	if err != nil {
...
func (m *Map[K, V]) set(funcName string, key K, value V, expiration time.Duration) error {
	...
	if _, err = m.storage.db.Exec(query, hashedKey, encKey, encValue, getKeyExpiresAt(expiration), nowUnixMilli()); err != nil {
...
func (m *Map[K, V]) mset(funcName string, pairs map[K]V, expiration time.Duration) error {
	...
	return execTransaction(m.storage.db, func(tx *sql.Tx) error {
		currCount := 0
		expiresAt := getKeyExpiresAt(expiration)
		now := nowUnixMilli()
...
func execSetBatch(tx *sql.Tx, tableName, funcName string, placeholders []string, args []interface{}) error {
	query := fmt.Sprintf(
		`
			INSERT INTO %s (key_hash, key, value, expires_at, updated_at)
         	VALUES %s
