...
func TestStorageOperations(t *testing.T) {
	...
	t.Run("Map", func(t *testing.T) {
		...
		expectedEntries := map[string]string{"name1": "value1", "name3": "value3"}
		...
		expectedKeys := []string{"name1", "name3"}
	...
	t.Run("Set", func(t *testing.T) {
		...
		hasExpired, err := tags.Has(randomTagValue)
	...
	t.Run("Queue", func(t *testing.T) {
		...
		expectedRemaining := []string{"send-notification-3", tempTask}
		if !reflect.DeepEqual(remainingTasks, expectedRemaining) {
		...
		expectedFinal := []string{"send-notification-3"}
		if !reflect.DeepEqual(finalTasks, expectedFinal) {
	...
	t.Run("Stack", func(t *testing.T) {
		...
		expectedRemaining := []string{"item1"}
		if !reflect.DeepEqual(remainingItems, expectedRemaining) {
