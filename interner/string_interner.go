...
func (si *StringInterner) Resolve(idx int32) string {
	if !si.resolve {
		panic("Resolve called on a StringInterner that was created with NewStringInternerOnly")
	}
...
func (si *StringInterner) Values() []string {
	if !si.resolve {
		panic("Values called on a StringInterner that was created with NewStringInternerOnly")
	}
	return si.values
}
